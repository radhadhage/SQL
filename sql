...
4) Write a query to get the employee ID, name (first_name, last_name), salary in ascending order of
salary.
Ans:- SELECT EMPLOYEE_ID,FIRST_NAME,LAST_NAME,SALARY FROM employees order by
salary;
...
Ans:- select first_name,last_name,department_id from employees where DEPARTMENT_ID in
(30,100) order by department_id;
...
Ans:- select first_name,last_name,salary from employees where salary > 10000 or salary<15000 and
...
first name.
Ans:- select FIRST_NAME from employees where FIRST_NAME like'%b%' or FIRST_NAME
like'%c%';
...
Ans:- select * from employees where JOB_ID in('IT_PROG','SH_CLERK')and SALARY not
in(4500,1000,1500);
...
11) Write a query to select all record from employees where last name in 'BLAKE', 'SCOTT',
'KING' and 'FORD'.
...
13) Write a query to get the job ID and maximum salary of the employees where maximum salary is
greater than or equal to $4000.
Ans:- SELECT job_id, MAX(salary) FROM employees GROUP BY job_id HAVING MAX(salary)
>=4000;

14) Write a query to get the average salary for all departments employing more than 10 employees.
Ans:- SELECT department_id, AVG(salary), COUNT(*) FROM employees GROUP BY
...
Ans:- Select first_name,Last_name,salary from employees where salary> (select salary from employees
where LAST_NAME='Bull');

2) Write a query to find the name (first_name, last_name) of all employees who works in the IT
department .
Ans:- select first_name ,last_name from employees where DEPARTMENT_ID in(select
DEPARTMENT_ID from departments where DEPARTMENT_NAME like'IT%');
...
Ans:-Select FIRST_NAME,Last_name from employees where MANAGER_ID!=0 and
DEPARTMENT_ID in(select DEPARTMENT_ID from departments where location_id in(select
LOCATION_ID from locations where COUNTRY_ID='US'));

4)Write a query to find the name (first_name, last_name) of the employees who are managers.
Ans:- select first_name,last_name from employees where EMPLOYEE_ID in(select MANAGER_ID
from employees);

5) Write a query to find the name (first_name, last_name), and salary of the employees whose salary
is greater than the average salary.
Ans:- select first_name,last_name,salary from employees where salary>(select avg(salary)from
employees);
...
Ans:- Select FIRST_NAME,Last_name,Salary from employees where salary=(select MIN_SALARY
...
Ans:- select first_name,last_name,salary from employees where salary>(select avg(salary) from
employees) and department_id in(select DEPARTMENT_ID from departments where
...
Ans:- SELECT first_name,LAST_NAME,SALARY FROM employees e1 where salary=(select
min(salary) from employees e2 where e2.DEPARTMENT_ID=e1.DEPARTMENT_ID);

10) Write a query to find the name (first_name, last_name) and salary of the employees who earn a
salary that is higher than the salary of all the Shipping Clerk (JOB_ID = 'SH_CLERK'). Sort the results
of the salary of the lowest to highest.
Ans:- select first_name,last_name,salary from employees where salary>(select max(salary) from
employees where JOB_ID='sh_clerk') order by salary;
...
12) Write a query to display the employee ID, first name, last name, and department names of all
employees.
Ans:- select first_name,last_name,(select DEPARTMENT_NAME from departments where
departments.DEPARTMENT_ID=employees.DEPARTMENT_ID ) As Depatment_Name from
employees;

13) Write a query to display the employee ID, first name, last name, salary of all employees whose
salary is above average for their departments.
Ans:- select EMPLOYEE_ID,FIRST_NAME,LAST_NAME,SALARY from employees where 
...
18) Write a query to list the department ID and name of all the departments where no employee is
working.
Ans:-SELECT DEPARTMENT_ID, DEPARTMENT_NAME FROM departments WHERE
...
2) Write a query to find the name (first_name, last name), department ID and department name of
all the employees.
Ans:- select FIRST_NAME,LAST_NAME,DEPARTMENT_ID,DEPARTMENT_NAME from
...
join locations l on l.location_id=d.location_id join jobs j on e.job_id=j.job_id where city='London';
...
hired after 'Jones'.
Ans:- select first_name,last_name,hire_date from employees where HIRE_DATE<(select HIRE_DATE
from employees where LAST_NAME='jones');

6) Write a query to get the department name and number of employees in the department.
Ans:-select department_name,count(*) from departments inner join employees on
employees.department_id=departments.department_id group by departments.department_id,department_name;
...
Ans:- select Employee_id,job_title,(jh.end_date-jh.start_date) from job_history jh natural join jobs where department_id=90;
...
Ans:- select DEPARTMENT_NAME,FIRST_NAME,CITY from employees e join departments d on
e.EMPLOYEE_ID=d.MANAGER_ID join locations l on l.LOCATION_ID=d.LOCATION_ID ;
...
13) Write a query to display department name, Department Manager (first_name, last_name), hire
date of manager, salary of the manager for those managers whose experience is more than 15 years.
Ans:- select department_name,first_name,last_name,hire_date,salary
from departments d join employees e on (d.manager_id=e.employee_id) where (DATEDIFF(now(),hire_date))/365>15
